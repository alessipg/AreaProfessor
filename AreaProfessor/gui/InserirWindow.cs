...
{
    public partial class InserirWindow : Form
    {
        public InserirWindow()
        {
            InitializeComponent();
                ...
                if (!txtNomeAluno.Text.Equals("") && !txtFase1.Text.Equals("") && !txtFase2.Text.Equals(""))
                {
                    if (VerificarFormatoData(txtData.Text))
                    {
                        if (VerificarFormatoHora(txtTempo.Text))
                        {
                            ...
                            {
                                DialogResult result = MessageBox.Show("Deseja cadastrar essa pontuação?", "Confirmação", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                                if(result == DialogResult.OK)
                                {
                                    ...
                                    new AreaProfessorDAO().cadastrar(pontuacao);
                                    MessageBox.Show("Pontuação cadastrada com sucesso.", "Mensagem", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    this.Close();
                                }
